...
data augmentation using glove vector

augmentation method from paper 
<EDA: Easy Data Augmentation Techniques for Boosting Performance on Text Classiﬁcation Tasks>

Created on Mon May 27 11:59:41 2019
...
cur_sent_embd_type='max'#['ave','max','concat','hier']
K=5 #top K results for evaluation

alpha=0.3 # data augmentaion param
EDA_method='SR' #'SR''RI''RS''RD'
n_aug=8 # number of augmented 
...
if((EDA_method=='SR')):
    for _asin in train_review_label_dic:
        reviews=train_review_label_dic[_asin][0]
        aug_reviews=[]
        for review in reviews:
            ...
            for i in range(n_aug):
                new_review=[]
                for sent in review:
                    new_sent=func_EDA.synonym_replacement(alpha,sent)
                    new_review.append(new_sent)
                aug_reviews.append(new_review)
        train_review_label_dic[_asin][0]=aug_reviews
        
#    for _asin in val_review_label_dic:
#        reviews=val_review_label_dic[_asin][0]
#        aug_reviews=[]
#        for review in reviews:
#            aug_reviews.append(review)
#            for i in range(n_aug):
...
#                    new_review.append(new_sent)
#                aug_reviews.append(new_review)
#        val_review_label_dic[_asin][0]=aug_reviews
    ...
    for _asin in train_review_label_dic:
        ...
        for review in reviews:
            ...
            for i in range(n_aug):
                new_review=[]
                for sent in review:
                    new_sent=func_EDA.random_insertion(alpha,sent)
                    new_review.append(new_sent)
                aug_reviews.append(new_review)
        train_review_label_dic[_asin][0]=aug_reviews
    ...
    for _asin in train_review_label_dic:
        ...
        for review in reviews:
            ...
            for i in range(n_aug):
                new_review=[]
                for sent in review:
                    new_sent=func_EDA.random_swap(alpha,sent)
                    new_review.append(new_sent)
                aug_reviews.append(new_review)
        train_review_label_dic[_asin][0]=aug_reviews
    ...
    for _asin in train_review_label_dic:
        ...
        for review in reviews:
            ...
            for i in range(n_aug):
                new_review=[]
                for sent in review:
                    new_sent=func_EDA.random_deletion(alpha,sent)
                    new_review.append(new_sent)
                aug_reviews.append(new_review)
        train_review_label_dic[_asin][0]=aug_reviews
...
for review in X_train_text:
    for sent in review:
        text = BeautifulSoup(sent, "lxml").get_text()
        all_texts.append(text)

    
for review in X_val_text:
    for sent in review:
        text = BeautifulSoup(sent, "lxml").get_text()
        all_texts.append(text)
